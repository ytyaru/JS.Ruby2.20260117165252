...
それと`culture/`の構造について考察した。将来の拡張を見据えた構造は以下。過剰だが責任分離の細分化を優先した結果である。
...
しかし上記は不当に過剰と思われる。そもそもUnicode文字セットだけでは多様な数学式を表現できない。MathMLなど高度な構造化が必要である。故にテキスト処理に過ぎない`charset/unicode/`や`culture/`配下で自然言語以外の文脈について考慮する必要はないと考える。
...
--|--
`part`|`part`
`paging`|`PageBreak`,`page-break.js`
`hr`|``
