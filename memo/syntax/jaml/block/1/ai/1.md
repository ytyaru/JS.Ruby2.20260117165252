...
私としては`jaml/lib/text/`の担当だと考えます。なぜなら改行コードやインデント文字が曖昧である問題は、あくまで「テキスト形式」の問題だからです。テキスト形式は改行やインデントのコードすら不定な、非常に適当な規格です。そもそも文字セットのコードに制御コードを入れること自体が間違っていると思います。文字セットは印字可能な文字だけの定義であるべきであり、改行コードは複数の文字の連なりで形成された文章や段落を意味する時に使うものだから、文字セットでなくテキスト形式になって、はじめて登場すべき概念のはずです。テキスト形式はテキスト形式として、規格化すべきですが、未だ存在しません。雑に文字セットとして扱われています。改行コードは文字ではなく文書を構造化するメタコードなのに。この既存の文字セット定義が既に責任分離できていないのです。そのせいで今こうして正しく責任分離できずに居ると私は考えています。これはASCII時代からの技術負債です。


`jaml/lib/converter/`は、Jaml/HTML/PDF/EPub/Markdown/AsciiDocなど各種具体的な形式を担当する責任区分です。

世間一般では、テキスト形式から発展した形式、たとえばHTMLやMarkdownについては、改行コードやインデントの扱いは、各自の実装依存になっています。

それ、おかしくないですか？　問題は各種テキスト派生形式ではなく、大元のベースであるテキスト形式に問題があります。私なら改行は`\n`、インデントは`\t`と定義したいです。でも、これらは元々タイプライター用のコードなので、それを使うこと自体が間違っています。UnicodeにあるLSで改行を表現するのが正しいのでしょう。インデントは、知りません。多分それを想定した文字があるはずです。

ただ、キーボードを打って入力できるコードにしないと機能しない場合がほとんどです。なので、いつまでもタイプライター用の制御コードを使い続けるというクソみたいな状況になっています。これは`jaml/lib/text/`の領域、入力段階の最初でせき止めるべきでしょう。
...
ようするにJaml案件の内部では以下のようにする。JSにとっては単なる`string`リテラル値だが、そこに一定のルールを定め、インタフェースを用意する。普通のテキストなら、この情報は持っているはずというものだけを用意します。それは文書という体裁が持つべき最低限の定義です。普通それらは曖昧ではなく明確であるべきですが、現状のコンピュータ業界では曖昧なので、ここで確定させます。それらの設定はユーザが任意に指定できます。それ以降、変更できず、共用しやすくなります。

```js
class Text {constructor(options={charset='utf8', newline='\n', indent='\t'})}
class JamlText extends Text {normalize()}
class HtmlText extends Text {normalize()}
class MarkdownText extends Text {}
class AsciiDocText extends Text {}
class CsvText extends Text {}
class JsonText extends Text {}
```

Jaml原稿として使う時は更にUnicodeや日本語における正規化もしたいです。しかしそれらの処理は非常に難解かつ複雑かつ重く、ローカライズ時という限定的な状況下でのみ行う処理のため、共通化せず、Textとは別の責任区分とします。JamlTextにします。

```
jaml/
    text/           Jaml内部においてテキストの規格を定める
        core/       Textクラス
            src/
                main.js         Text,JamlText,HtmlText,...
                newline/
                    main.js
                indent/
                    main.js
        ...
        super/      Text派生クラス
            jaml/
                src/
                    main.js     JamlText。Textを継承し、normalizer/src/format/jaml.jsを参照する
            html/
            markdown/
            asciidoc/
            csv/
...
なんか、スマートじゃないな。わざわざ型定義するのが冗長に見える。でも責任分離するとこうなる。責任分離においては妥協したくない。今回は対象外だが将来HTMLが入力になれば`normalize()`を呼び出すことになるし。これで良いはず。
