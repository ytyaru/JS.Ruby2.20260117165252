...
合致しない。自動化は偽りの排除ではなく本質でない仕事の自動化を指している。仕掛けによる喜びの増幅のほうは合致する。

自動化は文字通り仕事の自動化を指している。たとえばHTMLを書く時はHTMLテキストをキーボードでタイプするより、Markdownで書くほうがタイプ数が少なく書けるし、テキストエディタにおける視認性も高い。また、エディタはテキストエディタのほうが少ないメモリで稼働できて効率的だ。リッチエディタはボタン操作などでマウスを併用せねばならず操作効率が悪い。故にテキストエディタで読めて編集できる形式であるのが良いと言える。但しそれをバイナリデータで表現したほうがデータ通信効率は良い。故にテキスト形式とバイナリ形式の二種類の形式を用意し、それらを相互変換可能にすることで、状況に応じた効率化を実現できるようにしたい。

このように効率は労力や時間やデータ量を減らして文書作成（自己表現）できること全般を指している。じつに幅広い意味を持っている。

この効率に関しては既存のシステムにも存在する基本的な機能である。効率化は価値を生む。本質的でないつまらない作業を最大限に排除し、本質的な仕事にのみ集中できる。これにて創作活動の効率が最大化する。

たしかに私は偽り騙し奪うことで人生や仕事が無価値化したと分析している。但し、それを解決する方法までは提示できていなかった。果たしてこのプロジェクトでそれを解決できるのかも不明だ。あまりに大きな野望すぎて不可能だろう。

お前は無価値化の原因である偽りを排除すれば価値の生成を効率化できる、と考えたのだろう。それは正しいに違いない。もし私が「偽りを排除する」という方法について考えるなら盗作か。文書の著者を偽ることを防ぐ機構が欲しい。内容について偽りを排除することは技術的に困難だろう。著者を偽ることも防ぎきれないとは思う。しかし自分が書いた文書を著者単位で網羅すれば、その統一性から判断しやすいはずだ。あるいは、著作権の主張をせず、ネットに公開した時点で人間やAIにパクられるという現実を受け入れてCC0ライセンスで公開するとか。金儲けを考えるとメタクソ化するので、それを考えない方針をデフォルト値にするとか。
