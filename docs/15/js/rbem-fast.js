...
export class RbEmFast {
  ...
  static parse(text) {
    ...
    processedText = processedText.replace(regex, (
      ...
      // --- 1, 2, 3. ルビを持つパターン ---
      let base = emBase || pipeBase || shortBase;
      let rubyContent = emRuby || pipeRuby || shortRuby;
      ...
      // 親文字内の《《...》》を先に処理し、メタ文字チェックを行う
      let hasInvalidMetaInBase = false;
      const parsedBase = base.replace(/《《([^》]*?)》》/g, (m, emContent) => {
          if (emContent.includes('\n') || /[《》｜]/.test(emContent)) {
              hasInvalidMetaInBase = true;
          }
          return `<em class="bouten">${emContent}</em>`;
      });
      if (hasInvalidMetaInBase || /[《》｜]/.test(base.replace(/《《[^》]*?》》/g, ''))) {
          return match;
      }

      // ルビの中身のメタ文字チェック
      if (/[《》｜]/.test(rubyContent)) {
          // ただし、下ルビ記法のための先頭の `｜` は許容する
          if (!(rubyContent.startsWith('｜') && !/[《》｜]/.test(rubyContent.slice(1)))) {
              return match;
          }
      }
      ...
      if ((url && rubyParts.length >= 3) || (!url && rubyParts.length >= 3)) return match;
