...
class ShortRuby {
    static #REG = /([一-龠〇々〆ヶヵ仝〻〼ヿ]+)《(?!《)([^》]*)》/g;
    static parse(text) {

    }
}
class Enclose { // 《》で囲われた文字（ruby(パイプ省略系、パイプ付与系), em）

}
export class RbEmFast {
    parse(text) {
        let processedText = Escape.encode(text);
        const metaPos = [...text.matchAll(/[｜《》]/g)];
        return Escape.encode(processedText);
    }
}
