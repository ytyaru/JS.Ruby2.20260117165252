...
HELP_DESCRIPTION = "中間ファイルをマージして、指定された条件でフィルタリングを行います。"

HELP_EPILOG = """
入出力ファイル:
  only-unicode.tsv   [入力] unicode.tsvにのみ存在する文字
  only-heuristic.tsv [入力] heuristic.tsvにのみ存在する文字
  both.tsv           [入力] 両方のファイルに存在する文字
  merge.tsv          [出力] 統合リスト。デフォルトでカテゴリと由来を付与
